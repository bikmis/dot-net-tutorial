...
- Inner Join: The result set contains records from the left table that match the records from the right table.
   SELECT c.id, c.name, o.order_date
   FROM customer c 
   INNER JOIN order o
   ON c.id = o.customer_id

- Left Join: The left join returns all rows from the left table and matching rows from the right table.
   SELECT c.id, c.name, o.order_date
   FROM customer c 
   LEFT JOIN order o
   ON c.id = o.customer_id  

- Right Join: The right join returns all rows from the right table and matching rows from the left table.
   SELECT c.id, c.name, o.order_date
   FROM customer c 
   RIGHT JOIN order o
   ON c.id = o.customer_id

- Full (Outer) Join: The full join returns all rows from both left and right tables.
   SELECT c.id, c.name, o.order_date
   FROM customer c 
   FULL JOIN order o
   ON c.id = o.customer_id

- Cross Join: A cross join is used to generate a paired combination of each row of the first 
...
- Union: Joins result set of two or more SELECT statements.
